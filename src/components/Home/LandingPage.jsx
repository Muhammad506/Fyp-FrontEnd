...
const LandingPage = () => {
  ...
  return (
    <main
      className="relative flex h-screen 2xl:h-full font-poppins items-center justify-center mt-16 md:mt-14 lg:mt-0 overflow-hidden bg-cover bg-center"
        ...
        <Carousel activeIndex={index} onSelect={handleSelect} controls={false} indicators={false}>
          {carouselData.map((item) => (
            <Carousel.Item key={item.id}>
              <div className="flex md:flex-row h-screen lg:h-screen justify-center lg:justify-between text-left items-center p-8 lg:p-10 gap-4 2xl:gap-40">
                <div className="flex flex-col gap-4 w-full md:w-[70%] lg:w-[40%] xl:w-[35%] 2xl:w-[70%] text-left justify-center items-center ml-0 lg:ml-20 xl:ml-32 2xl:ml-0">
                  <h1 className="text-xl md:text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl tracking-wide md:tracking-tight font-extrabold text-white drop-shadow-md">
                    {item.title}
                  </h1>
                  <p className="md:text-lg text-base text-white text-justify">{item.description}</p>
                </div>
